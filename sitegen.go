...
// block -> subgroup -> files
type fileGroup map[string]map[string][]string

var (
	// Individual block calendars created by the generator always end with this suffix.
	// This is the ONLY reliable classifier we need for aggregated vs individual.
	individualBlockRe = regexp.MustCompile(`(?i)_Block\.ics$`)

	// Extract program prefix like DBBWL, DBWINFO, DBMAB, DBWI, DBING, ...
	blockRe = regexp.MustCompile(`^(DB[A-Z]+)`)

	// Accept both letter+digits (A03, B05, D04, ...)
	// and pure numeric cohorts (01, 02, 03, 04, 05, ...)
	// after the dash.
	//
	// Examples matched:
	//   DBBWL-A03
	...
	//   DBMAB-03
	//   DBWI-05
	classRe = regexp.MustCompile(`^(DB[A-Z]+)-([A-Z]?\d{2,3})`)
...
// Robust split:
// aggregated = everything that is NOT a single block file
func splitAggregated(names []string) (aggregated []string, individual []string) {
	for _, name := range names {
		if individualBlockRe.MatchString(name) {
...
func normName(name string) string {
	...
	n = dashCollapseRe.ReplaceAllString(n, "-")
	return n
}
...
func renderPage(path, title, subtitle string, blocks fileGroup, blockOrder []string, showToolbar bool, navToAll bool, navToIndex bool) error {
	...
	b.WriteString("Use Subscribe for webcal subscription or Copy URL to add the feed manually.")
	b.WriteString("</div>")
